...
class Product:
    ...
    def check_quantity(self, quantity) -> bool:
        ...
        """
        if quantity < 0:
            raise ValueError('Количество продукта не может быть меньше 0!')
        else:
...
class Cart:
    ...
    def add_product(self, product: Product, buy_count=1):
        ...
        # if product in self.products and product.check_quantity(buy_count) is True:
        #     self.products[product] = self.products[product] + buy_count
        # elif product not in self.products and product.check_quantity(buy_count) is True:
        #     self.products[product] = buy_count
        # else:
        #     raise ValueError('Недостаточно продуктов на складе для добавления в корзину!')
